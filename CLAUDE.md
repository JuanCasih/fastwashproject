...
5. **El equipo** (`#combo` — el id es heredado de una versión anterior, no renombrado) — título grande centrado en celeste, después un bloque de dos columnas (`.equipo-grid`, alineado arriba con `align-items: start` porque la columna derecha es más alta): a la izquierda el diagrama de instalación (`.equipo-diagram`); a la derecha, apilados dentro de `.equipo-col-right`, primero la lista de 5 datos de espacio/conexión (`.equipo-datos`) y debajo el **selector de color** (`.color-picker`): subtítulo, una imagen de vista previa más chica que antes (`.color-picker-preview`, `max-width: 260px`, ajustada al ancho de la columna — antes vivía suelta a todo el ancho del contenedor, ahora queda integrada visualmente con los datos técnicos), con fade corto de `.18s` al cambiar de color (`data-color-preview-img`, respeta `prefers-reduced-motion`), y una fila de 6 muestras clickeables (`.color-swatch`, botones reales con `aria-label` por color y `aria-pressed` para el estado activo, marcado con un anillo vía `box-shadow`). El color "Blanco" reutiliza `machine-mlm535` (la foto del hero, que ya es blanca) en vez de tener un archivo propio; los otros 5 son `maquina-azul/amarillo/verde/rojo/negro`. Lógica en `main.js` → `mountColorPicker`/`initColorPicker`/`setColorPreview`. Al apilarse bajo 1024px el orden es: diagrama, datos técnicos, selector de color (en ese orden en el DOM, no requiere `order` de CSS). Por último, a todo el ancho de la sección, el subtítulo de las etapas (`data-equipo-etapas-subtitle`) usa la clase `.profile-section-title` — el mismo tratamiento grande/centrado/celeste que el título de la sección, no el estilo chico de `.subsection-title` — seguido de las 4 etapas del ciclo como tarjetas (`.cycle-grid`/`.cycle-card`) y una línea de cierre centrada debajo. Ver la advertencia de naming de modelo más arriba. Datos en `manifest.js` → `equipo` (incluye `equipo.colorPicker`).
